...
const deleteFeedBtns = [
  _deleteFeedBtn,
  ...document.getElementsByClassName('delete-feed-button'),
]
...
const moreArticles = async e => {
  ...
  const response = await fetch(
    `${$SCRIPT_ROOT}/_more_articles?feed_id=${feedId}&last_item_id=${lastItemId}&sort_order=${sortOrder}&show_read=${showRead}&start_at=${startAt}`
  )
  const json = await response.json()
...
const showArticlePreview = async e => {
  ...
  const response = await fetch(
    `${$SCRIPT_ROOT}/_article_contents?id=${articleId}`
  )
  const json = await response.json()
...
delegator(
  '#main-content',
  '.article-preview, .article-preview h3, .article-preview span',
  'click',
  showArticlePreview
)
...
const shareArticle = async e => {
  try {
    await navigator.share({ url: e.currentTarget.linkUrl })
...
delegator(
  '#main-content',
  '.article-share-button, .article-share-button span',
  'click',
  shareArticle
)
